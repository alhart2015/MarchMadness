...
def run_pipeline(tag, drop_features, tuned_params_json):
    ...
    """
    suffix = f"_{tag}"  # NOTE: even baseline gets _baseline so canonical v4
                        # outputs (output/cv_per_season_v3.csv etc.) are not
                        # clobbered. Driver consumes suffixed paths only.
